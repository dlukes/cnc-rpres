...
To generate the presentation, press the *Knit* button in RStudio. (You could
also knit it manually with `knitr::knit("index.Rmd")`, but that only generates
`index.md`. In order to get the HTML, you have to run a fairly complex pandoc
command. If you're curious, inspect the output in the *R Markdown* tab after
...
A single-file version of the presentation, which can be easily opened locally
in a web browser without being served over HTTP (just by double-clicking the
file, on most systems), can be created by setting the `self_contained` option
in the YAML header to `true`. This makes it easier to share the presentation
with non-technical people over e-mail (if they need their own offline copy for
some reason; if not, just sharing a link to a version hosted online, as
explained previously, is much better). As mentioned on the previous slide
though, be aware that *some functionality* (e.g. speaker notes) *might become
unavailable.*
